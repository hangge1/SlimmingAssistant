...
test("Astro public site owns ResetLife MVP and keeps slimming assistant as an action tool", () => {
  ...
  assert.match(siteSource, /复位路径/);
  assert.match(siteSource, /复位社区/);
  assert.match(homePageSource, /状态识别/);
  ...
  assert.match(statePageSource, /data-dialogue-card/);
  assert.match(statePageSource, /第 1 轮/);
  assert.match(statePageSource, /resetlife:latest-path/);
  assert.match(statePageSource, /保存这次状态对话/);
  assert.match(pathPageSource, /复位路径/);
  assert.match(pathPageSource, /三大修复方向/);
