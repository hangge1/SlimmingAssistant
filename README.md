...
- `/` 是“复位”首页，定位为重建人生系统。
- `/state` 是卡片式状态识别页，状态对话后会生成最新复位路径。
- `/path` 是复位路径页，承接状态识别结果，并给出优先修复方向。
- `/psychology`、`/cognition`、`/action` 是复位路径里的三大修复方向：心理重建、认知重塑、行为重构。
- `/records` 是我的复位记录页，用来看到每一次状态识别、修复动作和复盘记录。
...
复位的产品硬约束单独维护在 `doc/product-constraints.md`，包括选择题优先、可选自定义输入、面对面对话式文案、卡片式状态对话、避免医疗化表达等规则。
